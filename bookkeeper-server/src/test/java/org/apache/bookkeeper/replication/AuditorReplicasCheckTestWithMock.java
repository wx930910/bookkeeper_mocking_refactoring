...
public class AuditorReplicasCheckTestWithMock extends BookKeeperClusterTestCase {
	private MetadataBookieDriver driver;

	public AuditorReplicasCheckTestWithMock() {
		super(1);
	...
	private class MockTestBookKeeperAdmin {
		public BookKeeperAdmin MockedTestBookKeeperAdmin;
		...
		public MockTestBookKeeperAdmin(BookKeeper bkc, StatsLogger statsLogger,
			...
			this.MockedTestBookKeeperAdmin = EasyMock.partialMockBuilder(BookKeeperAdmin.class).withConstructor(bkc, statsLogger)
					.addMockedMethod("asyncGetListOfEntriesOfLedger", BookieSocketAddress.class, long.class).createMock();
			mockAsyncGetListOfEntriesOfLedger();
			EasyMock.replay(this.MockedTestBookKeeperAdmin);
		}

		private void mockAsyncGetListOfEntriesOfLedger() {
			EasyMock.expect(
					this.MockedTestBookKeeperAdmin.asyncGetListOfEntriesOfLedger(EasyMock.anyObject(BookieSocketAddress.class), EasyMock.anyLong()))
					.andAnswer(() -> {
						BookieSocketAddress address = EasyMock.getCurrentArgument(0);
						long ledgerId = EasyMock.getCurrentArgument(1);
						...
						if (errorReturnValue != null) {
							...
							AvailabilityOfEntriesOfLedger availabilityOfEntriesOfLedger = returnAvailabilityOfEntriesOfLedger.get(address.toString(),
									Long.toString(ledgerId));
	...
	private TestStatsLogger startAuditorAndWaitForReplicasCheck(ServerConfiguration servConf, MutableObject<Auditor> auditorRef,
			MultiKeyMap<String, AvailabilityOfEntriesOfLedger> expectedReturnAvailabilityOfEntriesOfLedger,
			MultiKeyMap<String, Integer> errorReturnValueForGetAvailabilityOfEntriesOfLedger)
			throws MetadataException, CompatibilityException, KeeperException, InterruptedException, UnavailableException, UnknownHostException {
		...
		TestOpStatsLogger replicasCheckStatsLogger = (TestOpStatsLogger) statsLogger.getOpStatsLogger(ReplicationStats.REPLICAS_CHECK_TIME);

		final TestAuditor auditor = new TestAuditor(
				Bookie.getBookieAddress(servConf).toString(), servConf, bkc, true, new MockTestBookKeeperAdmin(bkc, statsLogger,
						expectedReturnAvailabilityOfEntriesOfLedger, errorReturnValueForGetAvailabilityOfEntriesOfLedger).MockedTestBookKeeperAdmin,
				true, statsLogger);
	...
	private void createClosedLedgerMetadata(LedgerManager lm, long ledgerId, int ensembleSize, int writeQuorumSize, int ackQuorumSize,
			Map<Long, List<BookieSocketAddress>> segmentEnsembles, long lastEntryId, int length, DigestType digestType, byte[] password)
			throws InterruptedException, ExecutionException {
		LedgerMetadataBuilder ledgerMetadataBuilder = LedgerMetadataBuilder.create();
		ledgerMetadataBuilder.withEnsembleSize(ensembleSize).withWriteQuorumSize(writeQuorumSize).withAckQuorumSize(ackQuorumSize).withClosedState()
				.withLastEntryId(lastEntryId).withLength(length).withDigestType(digestType).withPassword(password);
	...
	private void createNonClosedLedgerMetadata(LedgerManager lm, long ledgerId, int ensembleSize, int writeQuorumSize, int ackQuorumSize,
			Map<Long, List<BookieSocketAddress>> segmentEnsembles, DigestType digestType, byte[] password)
			throws InterruptedException, ExecutionException {
		LedgerMetadataBuilder ledgerMetadataBuilder = LedgerMetadataBuilder.create();
		ledgerMetadataBuilder.withEnsembleSize(ensembleSize).withWriteQuorumSize(writeQuorumSize).withAckQuorumSize(ackQuorumSize)
	...
	private void runTestScenario(MultiKeyMap<String, AvailabilityOfEntriesOfLedger> returnAvailabilityOfEntriesOfLedger,
			...
			int expectedNumLedgersHavingLessThanAQReplicasOfAnEntry, int expectedNumLedgersHavingLessThanWQReplicasOfAnEntry)
			throws CompatibilityException, UnavailableException, UnknownHostException, MetadataException, KeeperException, InterruptedException {
		...
		try {
			TestStatsLogger statsLogger = startAuditorAndWaitForReplicasCheck(servConf, auditorRef, returnAvailabilityOfEntriesOfLedger,
					errorReturnValueForGetAvailabilityOfEntriesOfLedger);
			checkReplicasCheckStats(statsLogger, expectedNumLedgersFoundHavingNoReplicaOfAnEntry, expectedNumLedgersHavingLessThanAQReplicasOfAnEntry,
	...
	private void checkReplicasCheckStats(TestStatsLogger statsLogger, int expectedNumLedgersFoundHavingNoReplicaOfAnEntry,
			int expectedNumLedgersHavingLessThanAQReplicasOfAnEntry, int expectedNumLedgersHavingLessThanWQReplicasOfAnEntry) {
		...
		assertEquals("NUM_LEDGERS_HAVING_NO_REPLICA_OF_AN_ENTRY guage value", expectedNumLedgersFoundHavingNoReplicaOfAnEntry,
				numLedgersFoundHavingNoReplicaOfAnEntryGuage.getSample());
		assertEquals("NUM_LEDGERS_HAVING_LESS_THAN_AQ_REPLICAS_OF_AN_ENTRY guage value", expectedNumLedgersHavingLessThanAQReplicasOfAnEntry,
				numLedgersHavingLessThanAQReplicasOfAnEntryGuage.getSample());
		assertEquals("NUM_LEDGERS_HAVING_LESS_THAN_WQ_REPLICAS_OF_AN_ENTRY guage value", expectedNumLedgersHavingLessThanWQReplicasOfAnEntry,
	...
	public void testReplicasCheckForBookieHandleNotAvailable() throws Exception {
		 ...
		 * will return BookieHandleNotAvailableException.
		 */
		...
		long ledgerId = 1L;
		createClosedLedgerMetadata(lm, ledgerId, ensembleSize, writeQuorumSize, ackQuorumSize, segmentEnsembles, lastEntryId, length, digestType,
				password);
		for (BookieSocketAddress bookieSocketAddress : bookieAddresses) {
			errorReturnValueForGetAvailabilityOfEntriesOfLedger.put(bookieSocketAddress.toString(), Long.toString(ledgerId),
	...
	public void testReplicasCheckForLedgersFoundHavingLessThanAQReplicasOfAnEntry() throws Exception {
		...
		returnAvailabilityOfEntriesOfLedger.put(bookieAddresses.get(1).toString(), Long.toString(ledgerId),
				AvailabilityOfEntriesOfLedger.EMPTY_AVAILABILITYOFENTRIESOFLEDGER);
		errorReturnValueForGetAvailabilityOfEntriesOfLedger.put(bookieAddresses.get(2).toString(), Long.toString(ledgerId),
				BKException.Code.BookieHandleNotAvailableException);
		numLedgersFoundHavingLessThanAQReplicasOfAnEntry++;
	...
	public void testReplicasCheckForLedgersFoundHavingLessThanWQReplicasOfAnEntry() throws Exception {
		...
		ledgerId = 3L;
		createNonClosedLedgerMetadata(lm, ledgerId, ensembleSize, writeQuorumSize, ackQuorumSize, segmentEnsembles, digestType, password);
		errorReturnValueForGetAvailabilityOfEntriesOfLedger.put(bookieAddresses.get(0).toString(), Long.toString(ledgerId),
				BKException.Code.NoSuchLedgerExistsException);
		returnAvailabilityOfEntriesOfLedger.put(bookieAddresses.get(1).toString(), Long.toString(ledgerId),
				new AvailabilityOfEntriesOfLedger(new long[] { 0, 1, 2, 3 }));
		returnAvailabilityOfEntriesOfLedger.put(bookieAddresses.get(2).toString(), Long.toString(ledgerId),
